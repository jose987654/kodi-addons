...
def get_access_token():
    ...
    while True:  # Main loop for retrying the entire process
        ...
        log(f"Access token in settings: {'access_token' in settings}")
        log(f"Access token value: {settings.get('access_token', 'None')[:20] if settings.get('access_token') else 'None'}")
        log(f"Retry flag: {settings.get('retry_auth', False)}")
        
        if 'access_token' in settings and settings['access_token']:
            access_token = settings['access_token']
            if 'refresh_token' in settings:
                refresh_token = settings['refresh_token']
            log("SUCCESS: Authentication completed successfully from QR dialog")
        elif settings.get('retry_auth', False):
            # User chose to retry, clear the retry flag and continue the loop
            log("RETRY: User chose to retry, clearing retry flag and restarting")
            settings['retry_auth'] = False
            save_dict(settings, data_file)
            log("Retry flag cleared, restarting authentication process")
            continue
        else:
            log("FAILED: Authentication failed or was cancelled")
            return None
...
def create_qr_code(verification_url, temp_path, size=400):
    ...
    try:
        ...
        log(f"QR Server URL: {qr_url}")
        
        log("Making request to QR Server API...")
        response = requests.get(qr_url, timeout=10)
        log(f"QR Server response status: {response.status_code}")
        log(f"QR Server response headers: {dict(response.headers)}")
        log(f"QR Server response content length: {len(response.content)} bytes")
        
        response.raise_for_status()
        
        log("Writing QR code to file...")
        with open(temp_path, 'wb') as f:
            f.write(response.content)
        
        log(f"SUCCESS: QR code saved to: {temp_path}")
        log("==================================================")
        log("QR CODE CREATION COMPLETED")
        log("==================================================")
        return True
        
    except Exception as e:
        log(f"ERROR: Failed to create QR code: {str(e)}", xbmc.LOGERROR)
        log("==================================================")
        log("QR CODE CREATION FAILED")
        log("==================================================")
        return False
...
class QRAuthDialogWithPolling(xbmcgui.WindowDialog):
    ...
    def __init__(self, qr_image_path, verification_url, user_code, device_code, interval, max_attempts):
        ...
        log("==================================================")
        log("INITIALIZING QR AUTH DIALOG WITH POLLING")
        log("==================================================")
        log(f"QR Image Path: {qr_image_path}")
        log(f"Verification URL: {verification_url}")
        log(f"User Code: {user_code}")
        log(f"Device Code: {device_code[:10]}...")
        log(f"Polling Interval: {interval}s")
    ...
    def start_polling(self):
        ...
        def poll_for_token():
            log("==================================================")
            log("STARTING BACKGROUND TOKEN POLLING")
            log("==================================================")
            log(f"Device Code: {self.device_code[:10]}...")
            log(f"Max Attempts: {self.max_attempts}")
            log(f"Interval: {self.interval}s")
            
            attempts = 0
            while attempts < self.max_attempts and not self.authenticated:
                attempts += 1
                log(f"POLLING ATTEMPT {attempts}/{self.max_attempts}")
                
                # Update status
                self.status_label.setLabel(f"Checking authorization... ({attempts}/{self.max_attempts})")
                
                log(f"Calling get_token() with device_code: {self.device_code[:10]}...")
                token_dict = get_token(self.device_code)
                log(f"Token response: {token_dict}")
                
                if 'error' in token_dict:
                    error_type = token_dict['error']
                    log(f"TOKEN ERROR: {error_type}")
                    ...
                    if error_type == 'authorization_pending':
                    ...
                    elif error_type == 'authorization_declined':
                        ...
                        log(f"Authentication error: {error_type}", xbmc.LOGERROR)
                        self.status_label.setLabel(f"Error: {error_type}")
                        # Don't break immediately, continue polling for a few more attempts
                        if attempts >= 5:  # Only break after 5 attempts with error
                            log("Too many errors, stopping polling")
                            break
                        time.sleep(self.interval)
                else:
                    ...
                    if access_token:
                        ...
                        log(f"Refresh Token: {refresh_token[:20] if refresh_token else 'None'}...")
                        
                        settings['access_token'] = access_token
                        if refresh_token:
                            settings['refresh_token'] = refresh_token
                        save_dict(settings, data_file)
                        log("Tokens saved to settings")
                        
                        self.authenticated = True
                        self.status_label.setLabel("Authentication successful! Closing...")
                        log("Closing dialog in 1 second...")
            ...
            if not self.authenticated:
                ...
                log("==================================================")
                log(f"Total attempts made: {attempts}")
        ...
        log("Starting background polling thread...")
        self.poll_thread = threading.Thread(target=poll_for_token)
        self.poll_thread.daemon = True
        self.poll_thread.start()
        log("Background polling thread started")
    ...
    def show_retry_dialog(self):
        ...
        log("Showing retry dialog to user...")
        if xbmcgui.Dialog().yesno("Seedr Authentication", retry_msg):
            log("USER CHOSE TO RETRY AUTHENTICATION")
            # Set a flag to indicate retry is needed
            settings['retry_auth'] = True
            save_dict(settings, data_file)
            log("Retry flag set to True")
        else:
            log("USER CANCELLED RETRY")
            settings['retry_auth'] = False
            save_dict(settings, data_file)
            log("Retry flag set to False")
        log("==================================================")
        log("RETRY DIALOG COMPLETED")
        log("==================================================")
...
def show_qr_code_dialog_with_polling(verification_url, user_code, device_code, interval, max_attempts):
    ...
    log("==================================================")
    log("STARTING QR CODE DIALOG WITH POLLING")
    log("==================================================")
    log(f"Verification URL: {verification_url}")
    log(f"User Code: {user_code}")
    log(f"Device Code: {device_code[:10]}...")
    ...
    try:
        ...
        log("Generating QR code using external service...")
        qr_image_loaded = create_qr_code(verification_url, qr_image_path, 400)
        log(f"QR code generation result: {qr_image_loaded}")
        
        if qr_image_loaded and os.path.exists(qr_image_path):
            log("SUCCESS: QR code generated, showing custom dialog with polling")
            ...
            log("QR dialog created, showing modal...")
            dialog.doModal()
            log("QR dialog modal closed")
            dialog.close()
            log("QR dialog closed")
            
        else:
            # Fallback to text-only dialog with polling
            log("ERROR: QR code image failed to load, showing text-only dialog with polling", xbmc.LOGWARNING)
        ...
        log("==================================================")
        log("QR CODE DIALOG PROCESS COMPLETED")
        log("==================================================")
        return True
