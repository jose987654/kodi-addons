...
def get_access_token():
    ...
    while True:  # Main loop for retrying the entire process
        ...
        # Show QR code dialog with fallback to text
        show_qr_code_dialog(verification_url, device_code_dict['user_code'])
...
def create_simple_bitmap(width, height, data):
    """Create a simple BMP image from binary data"""
    try:
        # BMP header
        file_size = 54 + (width * height * 3)  # 54 byte header + RGB data
        ...
        dib_header = struct.pack('<IiiHHIIiiII', 
            ...
            0,   # compression
            width * height * 3,  # image data size
            0, 0, 0, 0  # other fields
        )
        
        return bmp_header + dib_header + data
    except Exception as e:
        log(f"Error creating bitmap: {str(e)}", xbmc.LOGERROR)
        return None

def create_qr_code_local(verification_url, temp_path, size=400):
    """Create QR code locally using improved implementation"""
    try:
        log(f"Creating local QR code for: {verification_url}")
        
        # Improved QR-like pattern generator
        grid_size = 29  # Larger grid for better scanning
        cell_size = size // grid_size
        actual_size = grid_size * cell_size
        
        # Initialize pattern matrix
        pattern = [[0 for _ in range(grid_size)] for _ in range(grid_size)]
        
        # Add finder patterns (3 corners)
        def add_finder_pattern(pattern, x, y):
            for i in range(7):
                for j in range(7):
        ...
        add_finder_pattern(pattern, 0, 0)  # Top-left
        add_finder_pattern(pattern, 0, grid_size - 7)  # Top-right
        add_finder_pattern(pattern, grid_size - 7, 0)  # Bottom-left
        ...
        # Add data pattern based on URL
        url_bytes = verification_url.encode('utf-8')
        data_hash = hash(verification_url)
        
        for i in range(grid_size):
            for j in range(grid_size):
                # Skip finder patterns and timing patterns
                if ((i < 9 and j < 9) or 
                    (i < 9 and j >= grid_size - 8) or 
                    (i >= grid_size - 8 and j < 9) or
                    i == 6 or j == 6):
                    continue
                
                # Generate pseudo-random pattern based on URL and position
                byte_index = (i * grid_size + j) % len(url_bytes)
                bit_value = (url_bytes[byte_index] + data_hash + i + j) % 2
                pattern[i][j] = bit_value
        
        # Add format information (simplified)
        format_bits = [1, 0, 1, 0, 1, 0, 1, 0]  # Dummy format info
        ...
        # Convert pattern to bitmap data
        bitmap_data = bytearray()
        
        # BMP files are stored bottom-to-top
        for i in range(actual_size - 1, -1, -1):
            grid_row = i // cell_size
            for j in range(actual_size):
                grid_col = j // cell_size
                
                if 0 <= grid_row < grid_size and 0 <= grid_col < grid_size:
                    color = 0 if pattern[grid_row][grid_col] else 255
                else:
                    color = 255  # White padding
                
                # RGB (stored as BGR in BMP)
                bitmap_data.extend([color, color, color])
            
            # Pad row to multiple of 4 bytes
            padding = (4 - (actual_size * 3) % 4) % 4
            bitmap_data.extend([0] * padding)
        
        # Create BMP file
        bmp_data = create_simple_bitmap(actual_size, actual_size, bitmap_data)
        
        if bmp_data:
            with open(temp_path, 'wb') as f:
                f.write(bmp_data)
            
            log(f"Local QR-like code saved to: {temp_path}")
            return True
        else:
            log("Failed to create bitmap data", xbmc.LOGERROR)
            return False
            
    except Exception as e:
        log(f"Error creating local QR code: {str(e)}", xbmc.LOGERROR)
        return False

def create_qr_code_with_fallback(verification_url, temp_path, size=400):
    """Create QR code with local generation and external service fallback"""
    try:
        log(f"Attempting to create QR code for: {verification_url}")
        
        # First try local generation
        if create_qr_code_local(verification_url, temp_path, size):
            log("Successfully created local QR code")
            return True
        
        log("Local QR generation failed, trying external service", xbmc.LOGWARNING)
        
        # Fallback to external service
        encoded_url = quote(verification_url, safe='')
        qr_url = f"https://api.qrserver.com/v1/create-qr-code/?size={size}x{size}&data={encoded_url}&format=png"
        
        log(f"Downloading QR image from external service: {qr_url}")
        ...
        log(f"External QR image saved to: {temp_path}")
        return True
        
    except Exception as e:
        log(f"Error creating QR code (both local and external failed): {str(e)}", xbmc.LOGERROR)
        return False

def show_qr_code_dialog(verification_url, user_code):
    """Show custom dialog with QR code and fallback text"""
    try:
        # Create temporary file path for QR image
        temp_dir = xbmcvfs.translatePath('special://temp/')
        qr_image_path = os.path.join(temp_dir, 'seedr_qr_code.bmp')
        
        # Generate QR code (local first, external fallback)
        qr_image_loaded = create_qr_code_with_fallback(verification_url, qr_image_path, 400)
        
        # Prepare dialog content
        dialog_title = __language__(32100)  # "QR Code Authentication"
        
        if qr_image_loaded and os.path.exists(qr_image_path):
            # Show dialog with QR code image
            message_lines = [
                __language__(32102),  # "Scan this QR code with your mobile device to authenticate:"
                "",
            ...
            # Create custom dialog with image
            dialog = xbmcgui.Dialog()
            
            # First show the QR code image using ShowPicture
            log(f"Showing QR code image: {qr_image_path}")
            xbmc.executebuiltin(f'ShowPicture({qr_image_path})')
            
            # Then show the text dialog with instructions
            message = "\n".join(message_lines)
            dialog.ok(dialog_title, message)
            ...
            message = "\n".join(message_lines)
            xbmcgui.Dialog().ok(dialog_title, message)
